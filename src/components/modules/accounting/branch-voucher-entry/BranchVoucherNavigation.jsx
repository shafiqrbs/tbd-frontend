...
export default function BranchVoucherNavigation({
                                                    activeVoucher,
                                                    setActiveVoucher,
                                                    setReloadList,
                                                    reloadList,
                                                    ...
                                                    setLastVoucherDate,
                                                    setBranchList,setPrimaryLedgerHeadObject,submitForm,setBranchData
                                                }) {
        ...
        const fetchData = async () => {
            ...
            try {
                ...
                const branchList = await dispatch(
                    getIndexEntityData({
                        url: "domain/b2b/sub-domain/outlet",
                        param: {},
                    })
                );

                if (getIndexEntityData.fulfilled.match(branchList)) {
                    if (branchList.payload?.data && branchList.payload?.data.length > 0) {
                        const transformedData = branchList.payload?.data.map(type => {
                            return {'label': type.name, 'value': String(type.sub_domain_id)}
                        });
...
import { useTranslation } from "react-i18next";
import { useOutletContext } from "react-router-dom";
import {Box, Text, Card, ScrollArea, Loader, Center, TextInput, Tooltip, Skeleton} from "@mantine/core";
import { getIndexEntityData } from "../../../../store/core/crudSlice.js";
import classes from "../../../../assets/css/FeaturesCards.module.css";
import { IconInfoCircle, IconSearch, IconX } from "@tabler/icons-react";
...
export default function BranchVoucherNavigation({
    ...
    const didFetch = useRef(false); // Prevent multiple fetches in StrictMode
    ...
    const fetchData = async () => {
        ...
        try {
            // Fetch voucher list
            const result = await dispatch(getIndexEntityData({ url: "accounting/voucher/wise-ledger-details", param: {} }));
            if (getIndexEntityData.fulfilled.match(result)) {
                setAllVoucherList(result.payload?.data || []);
            } else {
                setError(t("FailedToFetchVouchers"));
            ...
            // Fetch last voucher date
            const lastDate = await dispatch(getIndexEntityData({ url: "accounting/voucher/last-date", param: {} }));
            if (getIndexEntityData.fulfilled.match(lastDate)) {
                setLastVoucherDate(lastDate.payload?.data || null);
            } else {
                setError(t("FailedToFetchVouchers"));
            ...
            // Fetch branch list
            const branchList = await dispatch(getIndexEntityData({ url: "domain/b2b/sub-domain/outlet", param: {} }));
            if (getIndexEntityData.fulfilled.match(branchList)) {
                if (branchList.payload?.data && branchList.payload?.data.length > 0) {
                    const transformedData = branchList.payload.data.map(type => ({
                        label: type.name,
    ...
    const filteredVoucherList = useMemo(() => {
        ...
        const keyword = debouncedSearchValue.trim().toLowerCase();
        if (!keyword) return allVoucherList;
        return allVoucherList.filter((voucher) => voucher.name?.toLowerCase().includes(keyword));
    }, [allVoucherList, debouncedSearchValue]);
            ...
            return (
                ...
                >
                    <Box
                        ...
                        onClick={() => {
                            if (!isDisabled) handleActiveVoucher(item);
                        }}
                    >
                        <Text size="sm" fw={500} color="black" pl={8}>
                            {item.name || t("UnnamedVoucher")}
