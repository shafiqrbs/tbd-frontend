...
function SalesAddCustomerModel(props) {
    const {t, i18n} = useTranslation();
    const showEntityData = useSelector((state) => state.crudSlice.showEntityData)
    const theme = useMantineTheme();

    const closeModel = () => {
        props.setAddCustomerModel(false)
    }

    return (
        <Modal opened={props.addCustomerModel} onClose={closeModel} title={t('AddCustomerInformation')} size="75%" overlayProps={{
...
export default SalesAddCustomerModel;
