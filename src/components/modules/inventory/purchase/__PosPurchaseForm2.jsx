...
export default function __PosPurchaseForm2(props) {
    const {
        isSMSActive,
        currencySymbol,
        tempCardProducts,
...
export default function __PosPurchaseForm2(props) {
    ...
    // Warehouse Options
    const warehouseDropdown = getCoreWarehouseDropdownData(); // ✅ Assumes this returns an array like [{ label: 'Main', value: '1' }]
        ...
        <form
            onSubmit={form.onSubmit((values) => {
                ...
                setTimeout(() => {
                    ...
                    setOrderProcess(null);
                    setWarehouseData(null);
                    setLoadCardProducts(true); // ✅ Reload
                }, 500);
            })}
        >
            <Box bg="white" p="xs" className="borderRadiusAll">
                                    ...
                                    return (
                                        <Select
                                            ...
                                            onChange={(newValue) => {
                                                ...
                                                setLoadCardProducts(true); // ✅ Refresh View
                                            }}
                                        />
                                    );
                                },
                                    ...
                                    return (
                                        <TextInput
                                            ...
                                            onChange={(e) => {
                                                ...
                                                const updated = data.map((p) =>
                                                    p.product_id === item.product_id
                                                        ? { ...p, quantity: val, sub_total: val * p.sales_price }
                                                        : p
                                                );
                                                localStorage.setItem("temp-purchase-products", JSON.stringify(updated));
                                                setLoadCardProducts(true); // ✅ sync UI
                                            }}
                                        />
                                    );
                                },
